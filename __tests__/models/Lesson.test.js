...
describe("Lesson", () => {
    ...
    describe("getLessonsForStudent", () => {
        it("should fetch lessons for a given student ID", async () => {
            const mockStudentId = 1;

            const mockSchoolDays = [
                { lesson1_id: 1, lesson2_id: 2, lesson3_id: 3, lesson4_id: 4, lesson5_id: 5 },
                { lesson1_id: 6, lesson2_id: 7, lesson3_id: 8, lesson4_id: 9, lesson5_id: 10 }
            ];

            const mockLessons = [
                { id: 1, subject_id: 1, confidence: 3, enjoyment: 2, subject_name: "Math" },
                { id: 2, subject_id: 2, confidence: 1, enjoyment: 2, subject_name: "Science" },
                { id: 3, subject_id: 3, confidence: 2, enjoyment: 3, subject_name: "English" },
                { id: 4, subject_id: 4, confidence: 2, enjoyment: 1, subject_name: "History" },
                { id: 5, subject_id: 5, confidence: 3, enjoyment: 1, subject_name: "Geography" },
                { id: 6, subject_id: 6, confidence: 2, enjoyment: 3, subject_name: "Physics" },
                { id: 7, subject_id: 7, confidence: 2, enjoyment: 2, subject_name: "Chemistry" },
                { id: 8, subject_id: 8, confidence: 1, enjoyment: 3, subject_name: "Biology" },
                { id: 9, subject_id: 9, confidence: 3, enjoyment: 1, subject_name: "Art" },
                { id: 10, subject_id: 11, confidence: 1, enjoyment: 3, subject_name: "Music" }
            ];

            db.query.mockResolvedValueOnce({ rows: mockLessons });

            SchoolDay.getSchoolDaysForStudent = jest.fn().mockResolvedValue(mockSchoolDays);

            const fetchedLessons = await Lesson.getLessonsForStudent(mockStudentId);

            expect(SchoolDay.getSchoolDaysForStudent).toHaveBeenCalledWith(mockStudentId);
            const expectedLessonIds = mockSchoolDays.reduce((acc, schoolDay) => {
                return acc.concat([
            ...
            expect(db.query).toHaveBeenCalledWith(expect.any(String), expectedLessonIds);
            expect(fetchedLessons).toEqual(mockLessons.map(lesson => new Lesson(lesson)));
        });
